...
def ai_move(tree: MCTS, board: TicTacToeBoard, rollouts: int) -> TicTacToeBoard:
    for i in range(rollouts):
        tree.do_rollout(board)
    return tree.choose(board)

def play_game_mvm():
    tree_x = MCTS(exploration_weight=EXPLORATION_X)
    tree_o = MCTS(exploration_weight=EXPLORATION_O)
    board = new_tic_tac_toe_board()
    if PRINT_BOARDS:
        print(board.to_pretty_string())
    while not board.terminal:
        if board.turn is True:
            board = ai_move(tree_x, board, ROLLOUTS_X)
            if PRINT_BOARDS:
                print("X mueve (MCST)")
                print(board.to_pretty_string())
        else:
            board = ai_move(tree_o, board, ROLLOUTS_O)
            if PRINT_BOARDS:
                print("O mueve (MCST)")
                print(board.to_pretty_string())
    if board.winner is True:
        print("Resultado: Gana X")
    elif board.winner is False:
        print("Resultado: gana O")
    else:
